"""
Spatial Seer — Supabase Hardware Data Exporter v3
Pulls rescan_num alongside all existing columns.
"""
...
def fetch_all_rows(client: Client) -> list[dict]:
    ...
    while True:
        resp = (
            ...
            .select("device_id, csv_dump, room_label, noise_type, location, rescan, rescan_num")
...
def build_scan_id(room_label, location, noise_type, rescan_num, scan_index) -> str:
    return f"{room_label}__{location}__noise{noise_type}__r{rescan_num}__scan{scan_index:02d}"
...
def main():
    ...
    for row in rows:
        ...
        rescan_num = int(row.get("rescan_num", 0))
        key        = (row.get("room_label"), row.get("location"), row.get("noise_type"), rescan_num)
        ...
        df.insert(0, "scan_id",    build_scan_id(key[0], key[1], key[2], rescan_num, scan_idx))
        ...
        df.insert(7, "rescan_num", rescan_num)
    ...
    print(f"Locations  : {combined['location'].nunique()} unique locations")
    print(f"\nRescan batch breakdown:")
    print(combined.drop_duplicates("scan_id").groupby(["rescan", "rescan_num"]).size().rename("scans"))
    print(f"\nSaved: {OUTPUT_CSV}")
